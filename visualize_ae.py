...
for epoch in range(training_epochs):
    ...
    if epoch % display_step == 0:
        print("Epoch:", '%d,' % (epoch + 1), "Cost:",
              "{:.9f}".format(avg_cost))
ae_test_cost = sess.run(ae.calc_total_cost(),
                        feed_dict={
                            ae.x: mnist.test.images,
                            ae.keep_prob: ae.in_keep_prob
                        })
...
for j in range(n):
    recover_xs = sess.run(ae.reconstruct(),
                          feed_dict={
                              ae.x: batch_xs,
                              ae.keep_prob: ae.in_keep_prob
                          })
    image_compare[0:img_size, j * img_size:(j + 1) *
                  img_size] = batch_xs[j].reshape([img_size, img_size])
    image_compare[img_size:2 * img_size, j * img_size:(j + 1) *
                  img_size] = recover_xs[j].reshape([img_size, img_size])
